...
**Note**  as versions approach Release the go from 7.x to 7.9 as an example
...
# List of Projects in 7.9 and 7.x
curl -s 'https://artifacts-api.elastic.co/v1/branches/7.x' | jq '.branch.builds[0].projects | keys '
curl -s 'https://artifacts-api.elastic.co/v1/branches/7.9' | jq '.branch.builds[0].projects | keys '
...
# Get Latest 7.9 Builds for filebeat
curl -s 'https://artifacts-api.elastic.co/v1/branches/7.9' | jq '.branch.builds[0].projects[].packages[] | select(.type == "tar").url' | grep linux | grep filebeat

# Get the last 3 builds 7.9 Builds for filebeat
curl -s 'https://artifacts-api.elastic.co/v1/branches/7.9' | jq '.branch.builds[0,1,2].projects[].packages[] | select(.type == "tar").url' | grep linux | grep filebeat
...
# Get Attributes for specfic build 7.9.0-c32dea82 then get build
curl -s 'https://artifacts-api.elastic.co/v1/branches/7.9' | jq '.branch.builds[] | select(.build_id == "7.9.0-c32dea82")' | head -n 20

# Get Metadata about 3 most recent 7.9 builds
curl -s 'https://artifacts-api.elastic.co/v1/branches/7.9' | jq '.branch.builds[0,1,2] | { branch, build_id, end_time, manifest_version, release_branch, start_time, version }'

# Get Filebeat Specific Build 7.9.0-c32dea82 tar
curl -s 'https://artifacts-api.elastic.co/v1/branches/7.9' | jq '.branch.builds[].projects[].packages[] | select(.type == "tar").url' | grep linux | grep 7.9.0-c32dea82 | grep filebeat

# Get Docker Images for that specfic build
curl -s 'https://artifacts-api.elastic.co/v1/branches/7.9' | jq '.branch.builds[].projects[].packages[] | select(.type == "docker").url' | grep linux | grep 7.9.0-7bee7813 | grep filebeat
```
